...
#include <lpt/coroutines/corostack.hpp>
...
struct promise_base : public lpt::coroframe<promise_base> {
    std::coroutine_handle<> parentHandle;   // handle to parent in the async call stack
    ...
    /* Set a scheduler for this promise and all its descendants along the async call stack
     */
    void setScheduler(Scheduler& s) {
        scheduler = &s;
        if (_child) { _child->setScheduler(s); }
    }
};
...
struct GetData {
    ...
    void await_suspend(std::coroutine_handle<PROMISE_T> h) {
        ...
        for (promise_base* it = promise; it != nullptr; it = it->_parent) {
            if (it->scheduler) { sched = it->scheduler; break; }
        }
        assert(sched);
        // propagate scheduler
        for (promise_base* it = promise; it != nullptr && !it->scheduler; it = it->_parent) {
            it->scheduler = sched;
        }
    ...
    int await_resume() {
        // When we wake up, the scheduler will have the data that we were waiting for
...
struct Async {
    struct promise_type : public promise_base {
        ...
        auto final_suspend() noexcept {
            ...
            struct ResumeCaller {
                ...
                ResumeCaller(std::coroutine_handle<> h) noexcept
                    :p(h)
                {}
    ...
    Async(std::coroutine_handle<promise_type> h)
        :self(h)
    {}
    ...
    auto await_suspend(std::coroutine_handle<OtherPromise_T> h_other) {
        ...
        //assert(self.promise()._parent == nullptr);
...
struct AsyncResult {
    ...
    AsyncResult(std::coroutine_handle<promise_type> h)
        :self(h)
    {}
...
Async<int> inner_function(int i) {
    std::cout << std::this_thread::get_id() << " In inner" << '\n';
...
Async<int> inner_ready() {
    co_return 3;
}

struct WrappedInt { int i; };

Async<WrappedInt> middle_function(int i) {
    std::cout << std::this_thread::get_id() << " In middle" << '\n';
    int const ii = co_await inner_function(i);
    std::cout << std::this_thread::get_id() << " inner returned " << ii << '\n';
    co_return WrappedInt{ .i = ii * co_await inner_ready()};
}

Async<int> outer_function(int i) {
    std::cout << std::this_thread::get_id() << " In outer" << '\n';
    WrappedInt const res = co_await middle_function(i);
    std::cout << std::this_thread::get_id() << " middle returned " <<  res.i << '\n';
    co_return res.i + 3;
}


int main() {
    std::cout << std::this_thread::get_id() << " *** Deciphering Coroutines 2 - Demo ***" << '\n';
    ...
    std::cout << std::this_thread::get_id() << " Result value is " << async_result.result() << '\n';
}
